...
VOID MLOG::DumpBufferToFile( struct MEMREF * reference, UINT64 numElements, THREADID tid )
{
    ...
    for (UINT64 i = 0; i < numElements; i++, reference++)
    {
        if (reference->ea != 0)
        {
            _ofile << reference->pc << (reference->read ? "  R " : "  W ") << reference->ea
                   << " : " << reference->tsc << endl;
        }
    }
}
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            ...
            for (UINT32 opIdx = 0; opIdx < INS_MemoryOperandCount(ins); opIdx++)
            {
                ...
                {
                    if (readSize == 0) readSize = INS_MemoryOperandSize(ins, opIdx);
                    else read2Size = INS_MemoryOperandSize(ins, opIdx);
            ...
            if (readOperandCount > 0)
            {
                INS_InsertFillBuffer(ins, IPOINT_BEFORE, bufId,
                                     ...
                                     IARG_UINT32, readSize, offsetof(struct MEMREF, size),
                                     IARG_BOOL, TRUE, offsetof(struct MEMREF, read),
                                     IARG_TSC, offsetof(struct MEMREF, tsc),
                                     IARG_END);
            }
            ...
            if (readOperandCount == 2)
            {
                INS_InsertFillBuffer(ins, IPOINT_BEFORE, bufId,
                                     ...
                                     IARG_UINT32, read2Size, offsetof(struct MEMREF, size),
                                     IARG_BOOL, TRUE, offsetof(struct MEMREF, read),
                                     IARG_TSC, offsetof(struct MEMREF, tsc),
                                     IARG_END);
            }
            ...
            if (writeOperandCount > 0)
            {
                INS_InsertFillBuffer(ins, IPOINT_BEFORE, bufId,
                                     ...
                                     IARG_UINT32, writeSize, offsetof(struct MEMREF, size),
