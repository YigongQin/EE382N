...
rhs_psi(float* ps, float* ph, float* U, float* ps_new, float* ph_new, \
  ...
  if ( (i>0) && (i<fnx-1) && (j>0) && (j<fny-1) ) {
       ...
       int B=C-fnx;
        ...
        // ============================
        float psx = ps[R]-ps[C];
        float psz = ( ps[T] + ps[T+1] - ps[B] - ps[B+1]  ) * 0.25f; //psipjp - psipjm;
        float phx = ph[R]-ph[C];
        float phz = ( ph[T] + ph[T+1] - ph[B] - ph[B+1]  ) * 0.25f;
        ...
        // ============================
        psx = ps[C]-ps[L];
        psz = ( ps[T-1] + ps[T] - ps[B-1] - ps[B]) * 0.25f; //psimjp - psimjm;
        phx = ph[C]-ph[L];
        phz = ( ph[T-1] + ph[T] - ph[B-1] - ph[B]) * 0.25f; 
        ...
        // top edge flux
        // ============================
        psx = ( ps[R] + ps[T+1] - ps[L] + ps[T-1]) * 0.25f; //psipjp - psimjp;
        psz = ps[T]-ps[C];
        phx = ( ph[R] + ph[T+1] - ph[L] - ph[T-1]) * 0.25f; //phipjp - phimjp;
        phz = ph[T]-ph[C];
        ...
        // bottom edge flux
        // ============================
        psx = ( ps[R] + ps[B+1] - ps[L] - ps[B-1]  ) * 0.25f; //psipjm - psimjm;
        psz = ps[C]-ps[B];
        phx = ( ph[R] + ph[B+1] - ph[L] - ph[B-1]  ) * 0.25f;
        phz = ph[C]-ph[B];
        ...
        float A2 = atheta(phx,phz);
        A2 = A2*A2;
        float gradps2 = (psx)*(psx) + (psz)*(psz);
