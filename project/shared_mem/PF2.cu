...
rhs_psi(float* ps, float* ph, float* U, float* ps_new, float* ph_new, \
  ...
  if ( (i>0) && (i<fnx-1) && (j>0) && (j<fny-1) ) {
       ...
       int T=C+fnx;
       int B=C-fnx;
       float psC = ps[C], phC=ph[C];
       float psR = ps[R], phR=ph[R];
       float psL = ps[L], phL=ph[L];
       float psT = ps[T], phT=ph[T];
       float psB = ps[B], phB=ph[B];
        ...
        float psipjp=( psC + psR + psT + ps[T+1] ) * 0.25f;
        float psipjm=( psC + psR + psB + ps[B+1] ) * 0.25f;
        float psimjp=( psC + psL + ps[T-1] + psT ) * 0.25f;
        float psimjm=( psC + psL + ps[B-1] + psB ) * 0.25f;

        float phipjp=( phC + phR + phT + ph[T+1] ) * 0.25f;
        float phipjm=( phC + phR + phB + ph[B+1] ) * 0.25f;
        float phimjp=( phC + phL + ph[T-1] + phT ) * 0.25f;
        float phimjm=( phC + phL + ph[B-1] + phB ) * 0.25f;
        ...
        float phzn = ( phT - phB ) * 0.5f;
        float psxn = ( psR - psL ) * 0.5f;
        float pszn = ( psT - psB ) * 0.5f;

        float A2 = atheta(phxn,phzn);
        A2 = A2*A2;
        float gradps2 = (psxn)*(psxn) + (pszn)*(pszn);
        float extra =  -cP.sqrt2 * A2 * phC * gradps2;
        ...
        float rhs_psi = ((JR-JL) + (JT-JB) + extra) * cP.hi*cP.hi + \
                   cP.sqrt2*phC - cP.lamd*(1.0f-phC*phC)*cP.sqrt2*(U[C] + Up);
