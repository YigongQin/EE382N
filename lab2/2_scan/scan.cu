...
__global__ void
check_repeat(int N, int* input, int* output){

     // check the repeated elements in the input array, output array is flag array
     int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
concur_write(int N, int N_true, int* input, int* output){
...
double cudaScan(int* inarray, int* end, int* resultarray)
{
    ...
    exclusive_scan(device_input, rounded_length, device_result);//exclusive_scan(device_input, end - inarray, device_result);
...
int find_repeats(int *device_input, int length, int N_true, int *device_output) {
    ...
    // check where repeat, write to device_input
    check_repeat<<< num_blocks, blocksize  >>>(length, device_output, device_input);

   // exclusive_scan(device_output, length, device_input);
   // cudaMemcpy(temp, device_input, length * sizeof(int),
     //          cudaMemcpyDeviceToHost);
   // for (int i = 0; i < N_true; i++){printf("%d ",temp[i]);} 
    // scan the flag array device_input
    exclusive_scan(device_output, length, device_input);
    // write to the device_output simultaneously
    concur_write<<< num_blocks, blocksize  >>>( length,  N_true, device_input, device_output);//, &num_repeat);
...
double cudaFindRepeats(int *input, int length, int *output, int *output_length) {
    ...
    cudaMemcpy(device_output, input, length * sizeof(int),
               cudaMemcpyHostToDevice);
