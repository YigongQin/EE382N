...
__global__ void kernelRenderCircles_shared_mem_skip(int* separators, int num_total_blocks, int num_blockx, int num_blocky, int numPartitions) {
    ...
    else if (totalCircles > 150) {startPlace = 100;}
...
CudaRenderer::render() {
   ...
   // int* check_sps = new int[num_total_blocks];
    ...
    // select different render based on the total number of circles
    if (numCircles < 5){
        // rgb and rgby
        ...
        // assign different skip number for diferent number circles over one box  
