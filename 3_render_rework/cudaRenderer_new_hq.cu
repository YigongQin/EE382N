...
__global__ void kernelRenderCircles_shared_mem(int* separators, int num_total_blocks, int num_blockx, int num_blocky, int numPartitions) {
    ...
    // int totalCircles = Out_numCirclesPerPixel[numPixels] + numCirclesPerPixel[numPixels];
    // separators[blockId] = Out_numCirclesPerPixel[numPixels-1]+ numCirclesPerPixel[numPixels-1];
    // __syncthreads();
